...
1. **Bookmark naming convention:** `_ch1`, `_ch2`, `_appendix_a`, etc.

2. **Create right-aligned tab stop** with dot leader at text area width:
...
1. **After each TOC entry**, add tab + `PAGEREF` field code using `w:fldChar` (begin/separate/end) + `w:instrText`:
...
1. **PAGEREF resolves** when the user opens in Word and updates fields: **Ctrl+A → F9**
2. python-docx **cannot calculate page numbers** — only Word's layout engine can
...
1. **No page numbers, no dot leaders** — ebook readers handle navigation natively
...
1. **Recommended build script pattern:**
