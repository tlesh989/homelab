...
Headroom proxy at localhost:8787 compresses CC tool outputs by ~34%, extending effective context window. `ANTHROPIC_BASE_URL` is set automatically via a fish alias in `~/.config/fish/config.fish` — no manual export needed.
...
- Terminal 2: open a new terminal and run `claude` (fish alias injects `ANTHROPIC_BASE_URL` automatically)
